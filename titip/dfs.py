        ...
        if current not in visited:
            ...
            for child in reversed(children):  # reverse untuk LIFO
                if child not in visited:
                    relation = f"anak ke-{len(descendants) + 1} dari generasi {level}"
...
def find_person_dfs(family_tree, target_person, max_level=20):
    ...
    # Temukan semua leluhur tertinggi (orang tanpa ayah)
    top_ancestors = [person for person, data in family_tree.items() if data.get('father') is None]
